...
// ─── CORS preflight — ОТДЕЛЬНЫЙ обработчик, иначе OPTIONS → 405 ───
...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    // Rate limiting (in-memory, сбрасывается при cold start)
...
async function sendToTelegram(data: Record<string, any>, env: Env): Promise<boolean> {
